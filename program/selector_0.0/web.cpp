...
TstateReadTag TWebServer::readTag( File f, String& tag){    // возвращает тип результата чтения TstateReadTag
  ...
  if( f.peek() == '<' ){
    ...
    // проверяем что это textarea и в этом случае корректируем тип тэга
    if( tag.indexOf("textarea") > 0 )  { typeRes = TagTextArea; };
        ...
        res = false;
          ...
          while( state != endOfFile ){   //пока не конец файла            
            ...
            case TagTextArea:{
              ...
              // читаем очередную строку лога и отправляем клиенту в браузер
              while( readLn( logFile, _tag ) ){ _client.println(_tag);  };
              logFile.close();
              break;
            };  
            case Text:{
