#pragma once
#include <Arduino.h>
//#include "common.h" // только из-за константы  T_DELAY
...
enum  TNameCounters { Any = 0,  Tping = 1,   LockSwitch = 2, returnA = 3,  press3s = 4, oneHour = 5, timeOutWeb = 6, SyncTimeSrv = 7,   };     // названия счетчиков, CountX - неиспользуемый счетчик
 /* 
 ...
 oneHour - счетчик на час для выполнения вспомогательных операций ( проверка даных, запись в память и т.д)
...
*/
 ...
 void load(  TNameCounters i, long int t);        // установка нового предельного значения t для счетчика name
